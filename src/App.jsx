...
const TEAM_DATA = {
  es: [
    { initials:"LP", name:"Lorena Paredes",    role:"Gerente General",          grad:"#003A4F,#006E8C", photo:LORENA_PHOTO,   photoPos:"center 20%" },
    { initials:"GE", name:"Giovanni Escalante", role:"Director Comercial",       grad:"#0094B6,#006E8C", photo:GIOVANNI_PHOTO, photoPos:"center center" },
    { initials:"HA", name:"Héctor Aguirre",     role:"Gerente de Logística",     grad:"#7FD4E8,#0094B6", photo:HECTOR_PHOTO,   photoPos:"center 20%" },
    { initials:"CG", name:"Christian González", role:"Supervisor de Logística",  grad:"#00C4E8,#0094B6", photo:"https://res.cloudinary.com/dlmoudikf/image/upload/v1774272310/WhatsApp_Image_2026-03-21_at_14.35.55_eidw9p.jpg", photoPos:"center 20%" },
    { initials:"DS", name:"David Soto",         role:"Asistente Administrativo", grad:"#003A4F,#0094B6", photo:DAVID_PHOTO,    photoPos:"center 20%" }
  ],
  en: [
    { initials:"LP", name:"Lorena Paredes",    role:"General Manager",          grad:"#003A4F,#006E8C", photo:LORENA_PHOTO,   photoPos:"center 20%" },
    { initials:"GE", name:"Giovanni Escalante", role:"Commercial Director",      grad:"#0094B6,#006E8C", photo:GIOVANNI_PHOTO, photoPos:"center center" },
    { initials:"HA", name:"Héctor Aguirre",     role:"Logistics Manager",        grad:"#7FD4E8,#0094B6", photo:HECTOR_PHOTO,   photoPos:"center 20%" },
    { initials:"CG", name:"Christian González", role:"Logistics Supervisor",     grad:"#00C4E8,#0094B6", photo:"https://res.cloudinary.com/dlmoudikf/image/upload/v1774272310/WhatsApp_Image_2026-03-21_at_14.35.55_eidw9p.jpg", photoPos:"center 20%" },
    { initials:"DS", name:"David Soto",         role:"Administrative Assistant", grad:"#003A4F,#0094B6", photo:DAVID_PHOTO,    photoPos:"center 20%" }
  ]
};
      ...
      <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:28,padding:"0 48px 60px",maxWidth:1200,margin:"0 auto"}}>
        {TEAM.map((m,i)=>(
          <div key={i} style={{background:"white",borderRadius:20,padding:"32px 24px",textAlign:"center",width:200,boxShadow:"0 2px 16px rgba(0,58,79,0.07)",border:"1px solid rgba(0,110,140,0.06)",transition:"transform .3s,box-shadow .3s"}}
            ...
            onMouseLeave={e=>{e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="0 2px 16px rgba(0,58,79,0.07)"}}>
            {m.photo
              ? <img src={m.photo} alt={m.name} style={{width:80,height:80,borderRadius:"50%",margin:"0 auto 16px",objectFit:"cover",objectPosition:m.photoPos||"center center",boxShadow:"0 4px 16px rgba(0,148,182,0.3)",display:"block"}}/>
...
function FleetPage({ nav, lang }) {
  const t = T[lang];
  const PHOTOS_EMPAQUE = [
    "https://res.cloudinary.com/dlmoudikf/image/upload/v1774483911/WhatsApp_Image_2026-03-17_at_16.50.51_rwxcdp.jpg",
    "https://res.cloudinary.com/dlmoudikf/image/upload/v1774483912/istockphoto-1295220494-612x612_rlr801.jpg",
    "https://res.cloudinary.com/dlmoudikf/image/upload/v1774483912/WhatsApp_Image_2026-03-17_at_17.03.20_beht8d.jpg"
  ];
    ...
    <div style={{background:"#F4FBFD", minHeight:"80vh"}}>
      ...
      <div style={{maxWidth:1200,margin:"0 auto",padding:"0 48px 60px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:20,marginBottom:64}}>
          {PHOTOS_EMPAQUE.map((photo,i)=>(
            photo
              ? <img key={i} src={photo} alt={`Empaque ${i+1}`} style={{width:"100%",height:435,objectFit:"cover",objectPosition:i===0?"center top":"center center",borderRadius:16,boxShadow:"0 2px 12px rgba(0,58,79,0.08)",display:"block"}}/>
...
export default function App() {
  ...
  return (
    <div style={{fontFamily:"DM Sans,system-ui,sans-serif"}}>
      ...
      <style>{`@media(min-width:768px){.nav-links{display:flex!important}.hamburger{display:none!important}}`}</style>

      {/* NAV */}
      <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,background:"rgba(255,255,255,0.97)",backdropFilter:"blur(12px)",borderBottom:"1px solid rgba(0,110,140,0.12)",boxShadow:"0 2px 24px rgba(0,110,140,0.08)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 20px",height:72}}>
          {/* Logo */}
          <div onClick={()=>nav("home")} style={{display:"flex",alignItems:"center",cursor:"pointer",flexShrink:0}}>
            <img src={LOGO} alt="Grupo G&M" style={{height:52,width:"auto",objectFit:"contain"}}/>
          </div>

          {/* Desktop nav links */}
          <div className="nav-links" style={{display:"none",gap:2,flex:1,justifyContent:"center"}}>
            {Object.entries(pages).map(([id,label])=>(
              <button key={id} onClick={()=>nav(id)} style={{background:page===id?"#0094B6":"transparent",color:page===id?"white":"#6B7280",border:"none",padding:"8px 0",borderRadius:100,fontSize:13,fontWeight:500,cursor:"pointer",fontFamily:"inherit",transition:"all .25s",whiteSpace:"nowrap",minWidth:{home:72,services:88,about:148,team:132,catalogs:88,fleet:148,terms:80}[id]||100}}>{label}</button>
            ))}
          </div>

          {/* Right side: lang + hamburger */}
          <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
            <div style={{display:"flex",gap:4,background:"#F4FBFD",borderRadius:100,padding:"3px 4px",border:"1px solid rgba(0,110,140,0.15)"}}>
              <button onClick={()=>setLang("es")} style={{background:lang==="es"?"#0094B6":"transparent",color:lang==="es"?"white":"#6B7280",border:"none",padding:"5px 10px",borderRadius:100,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit",transition:"all .2s",width:54}}>🇬🇹 ES</button>
              <button onClick={()=>setLang("en")} style={{background:lang==="en"?"#0094B6":"transparent",color:lang==="en"?"white":"#6B7280",border:"none",padding:"5px 10px",borderRadius:100,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit",transition:"all .2s",width:54}}>🇺🇸 EN</button>
            </div>
            {/* Hamburger button - only on mobile */}
            <button className="hamburger" onClick={()=>setMenuOpen(!menuOpen)} style={{background:"transparent",border:"none",cursor:"pointer",padding:8,display:"flex",flexDirection:"column",gap:5}}>
              <span style={{display:"block",width:22,height:2,background:"#006E8C",borderRadius:2,transition:"all .3s",transform:menuOpen?"rotate(45deg) translate(5px,5px)":"none"}}/>
              <span style={{display:"block",width:22,height:2,background:"#006E8C",borderRadius:2,transition:"all .3s",opacity:menuOpen?0:1}}/>
              <span style={{display:"block",width:22,height:2,background:"#006E8C",borderRadius:2,transition:"all .3s",transform:menuOpen?"rotate(-45deg) translate(5px,-5px)":"none"}}/>
            </button>
          </div>
        </div>
        ...
        {menuOpen && (
          <div style={{background:"white",borderTop:"1px solid rgba(0,110,140,0.1)",padding:"12px 20px 20px",display:"flex",flexDirection:"column",gap:4}}>
            {Object.entries(pages).map(([id,label])=>(
              <button key={id} onClick={()=>nav(id)} style={{background:page===id?"#0094B6":"transparent",color:page===id?"white":"#374151",border:"none",padding:"12px 16px",borderRadius:10,fontSize:15,fontWeight:500,cursor:"pointer",fontFamily:"inherit",textAlign:"left",transition:"all .2s"}}>{label}</button>
            ))}
          </div>
        )}
      </nav>
